...
Future<void> checkForVulnerabilities(String jsonFilePath, BuildContext context) async {
  try {
    ...
    // TODO: Add functionality to fully search apps
    ...
    for (var app in jsonData['apps']){
        ...
        if (cpe != null) {
          cpeAppList.add(cpe);
          cpeUrls.add('https://services.nvd.nist.gov/rest/json/cpes/2.0?cpeMatchString='+cpe);
        }
    }

    for (var url in cpeUrls){
      var cpeAppResponse = await http.get(Uri.parse(cpeUrl));
      var cpeAppData = json.decode(cpeResponse.body);
      print('===APP CPE STRING: $url===');
    }
...
String packageNameToCpe(String packageName, String version) {
  ...
  if (parts.length > 2) {
    ...
    String product = parts.sublist(2).join('_');
